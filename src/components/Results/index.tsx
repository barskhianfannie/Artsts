...
import { useDune, useDuneResults, useDuneStatus } from "../../hooks/useDune";
import { ScatterChart, Scatter, XAxis, 
  YAxis, CartesianGrid, Tooltip, Legend, ZAxis } from 'recharts';
...
export const DuneResults = ({executionId}:{executionId:string}) => {
  ...
  const handleScatter = async ()=>{
    ...
    if(priceInfo !== undefined){
      console.log('priceInfoEx')

    const {original:originalPts, predicted: predictedPts } = await Sequential({data:priceInfo})
  ...
  if(priceInfo===undefined){
    return (<><div style={{display:'flex', justifySelf:'center'}}><Image alt={'loading'} height={50} width={50} src={eth}/></div></>)
  }else
    if(chart===undefined)
{
return (<> 
    <h3
        ...
        >
          ...
          >
            {fetching ? 'Training....' : 'Train'}
          </button>
          </div>
          {fetching && <div style={{            justifyContent: "center",
            display: "flex"}}><Image alt={'loading'} height={90} width={90} src={eth}/></div>}
          </>
          )
  }else{
          ...
          <div style={{            justifyContent: "center",
            display: "flex"}}><Image alt={'loading'} height={90} width={90} src={eth}/></div>
    {chart && <div style={{display:'block', justifyContent:'center'}}>
<h5 style={{            fontFamily: "sans-serif",
    ...
    <CartesianGrid strokeDasharray="3 3" />
            ...
            <Scatter data={chart2} fill={'gray'} name='Predicted Values' attributeName="Predicted"/>
            <ZAxis   />
